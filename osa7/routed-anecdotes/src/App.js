...
const CreateNew = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    navigate('/')
    props.setNotify(`new ${content.value} was created!`)
    setTimeout(()=>props.setNotify(''), 5000)
  }
...
const App = () => {
  ...
  const Anecdote = ({anecdotes}) => {
    ...
    return (
      <div>
        ...
        <div><a href={note.info}>{note.info}</a></div>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      {notify}
      <Routes>
        <Route path="/Create" element={<CreateNew addNew={addNew} setNotify={setNotify} />} />
